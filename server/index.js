...
app.post('/subscribe', async (req, res) => {
    ...
    try {
        // Save to Supabase (we'll create this table soon)
        ...
        res.status(201).json({ message: 'Subrolled successfully' });
    } catch (error) {
...
app.post('/notify-new-order', async (req, res) => {
    const order = req.body.record; // Supabase webhook sends the record here
    ...
    try {
        ...
        const payload = JSON.stringify({
            ...
            icon: '/logo.png', // Assuming logo.png is in dashboard public
            data: {
                url: '/orders',
                orderId: order.id
            }
        ...
        const sendPromises = subscriptions.map(sub => {
            ...
            return webPush.sendNotification(subData, payload)
                .catch(err => {
                    ...
                    // If sub is expired/invalid, we should delete it
